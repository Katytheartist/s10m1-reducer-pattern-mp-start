...
case DELETE_QUOTE:
  return {...state, quotes: state.quotes.filter(qu=> qu.id != action.payload)} 
case EDIT_QUOTE_AUTHENTICITY:
  return {...state, 
    quotes: state.quotes.map(qu=> {
    if (qu.id != action.payload) return qu
    return {...qu, apocryphal: !qu.apocryphal}
    }) 
  }
